"use client";

import { Modal } from "@/components/modals/modal";
import { StatusBadge } from "@/components/ui";
import type { MemberRow } from "../data";

type MemberDetailModalProps = {
  member: MemberRow;
  onClose: () => void;
};

function DetailItem({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[18px] border border-border-soft bg-bg-muted px-4 py-4">
      <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-text-subtle">
        {label}
      </p>
      <p className="mt-2 text-[14px] font-medium text-text-primary">{value}</p>
    </div>
  );
}

export function MemberDetailModal({
  member,
  onClose,
}: MemberDetailModalProps) {
  return (
    <Modal
      title={member.name}
      onClose={onClose}
      panelClassName="max-w-[720px]"
      bodyClassName="mt-6 space-y-6"
    >
      <div className="flex flex-col gap-3 rounded-[20px] border border-border-soft bg-bg-muted px-5 py-5 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-[18px] font-semibold text-text-primary">
            {member.email}
          </p>
          <p className="mt-1 text-[14px] text-text-secondary">{member.phone}</p>
        </div>
        <StatusBadge label={member.status} tone={member.tone} />
      </div>

      <div className="grid gap-4 sm:grid-cols-2">
        <DetailItem label="Member ID" value={member.id} />
        <DetailItem label="Joined date" value={member.joinedDate} />
        <DetailItem label="Plan" value={member.plan} />
        <DetailItem label="Branch" value={member.branch} />
        <DetailItem label="Last visit" value={member.lastVisit} />
        <DetailItem label="Expiry date" value={member.expiryDate} />
      </div>
    </Modal>
  );
}
