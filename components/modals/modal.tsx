...
export function Modal({
  ...
  useEffect(() => {
    ...
    const autofocusTarget = focusableElements.find(
      (element) => element.dataset.autofocus === "true",
    );
  ...
  return (
    <div
      className={clsx(
        "fixed inset-0 z-[140] flex items-center justify-center bg-bg-overlay px-4 py-8",
        overlayClassName,
      )}
      onMouseDown={(event) => {
    ...
    >
      <div
        ...
        className={clsx(
          "relative max-h-[calc(100dvh-2rem)] w-full max-w-[573px] overflow-auto rounded-[24px] bg-bg-surface p-8 shadow-[var(--shadow-panel)]",
          panelClassName,
        )}
      >
        <div className="flex items-center justify-between gap-4">
          <h2
            id={titleId}
            className={clsx(
              "text-[20px] font-semibold leading-[1.4] text-text-primary md:text-[24px]",
              titleClassName,
            )}
          >
            {title}
